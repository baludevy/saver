...
const command: Command = {
  ...
  async execute(i: ChatInputCommandInteraction) {
    ...
    let msg = await i.deferReply({ flags: MessageFlags.Ephemeral});

    try {
      await i.editReply({ content: "downloading media..." });
      ...
      if (await shouldCompress(inputPath)) {
        await i.editReply({
          content: "compressing video to fit upload limit...",
        });
      ...
      await i.editReply({ content: "uploading to discord..." });
      ...
      await i.editReply({ content: `an error occured: ${err}` });
    } finally {
